        ...
        if ban and ban.is_active():
            ...
            if ban.ban_type == BAN_TYPE_PERMANENT:
                ...
                if remaining:
                    ...
                    if hours > 0:
                        time_str = _("${hours} hours and ${minutes} minutes", 
                    ...
                    message = _("You are temporarily banned from commenting for ${time}. Reason: ${reason}",
                              mapping={"time": time_str, "reason": ban.reason or _("No reason provided")})
                else:
                    message = _("Your comment ban has expired. Please refresh the page.")
            else:
...
def process_shadow_banned_comment(comment, context):
    ...
    """
    # Get comment author
    author_id = getattr(comment, 'creator', None) or getattr(comment, 'author_username', None)
    if not author_id:
